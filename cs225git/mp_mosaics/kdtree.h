...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     int partition(vector<Point<Dim>> &list,int dim, int left, int right);
     Point<Dim>& quickselect(vector<Point<Dim>> &list,int dim,int left,int right,int k);
     typename KDTree<Dim>::KDTreeNode* buildTree(vector<Point<Dim>> &list,int dim,int left, int right);
     //Point<Dim> find_helper(KDTreeNode *subroot,Point<Dim>& query,int dim) const;
     void findNN_helper( const Point<Dim>& query, Point<Dim> & currentBest, KDTreeNode * subroot, int dim ) const;
     typename KDTree<Dim>::KDTreeNode* copy(KDTreeNode *otherroot);
     void destroy(KDTreeNode *myroot);
};
