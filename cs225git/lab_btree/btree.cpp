...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     */
     ...
     }else{  //recusive the key on the child node
       if (subroot->is_leaf){
         return V();
       }
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    auto child_itr = parent->children.begin() + child_idx + 1; //pt 2
    /* Iterator for where we want to insert the new element. */
    auto elem_itr = parent->elements.begin() + child_idx;  //pt 1
    ...
    //step1: insert the mid element from the child into its new position in the
    // parent's elements.
    parent->elements.insert(elem_itr,*mid_elem_itr);
    //step2: insert newchild node for |6|
    parent->children.insert(child_itr,new_right);
    //step2: copy |8| and its child to new BTreeNode-new_right
    new_right->elements.assign(mid_elem_itr + 1, child->elements.end());
    new_right->children.assign(mid_child_itr, child->children.end());
    //step3: erase |6|8| and its children from original BTreeNode-child
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    //case1: if the pair exists in BTree, done!
    if (first_larger_idx < subroot->elements.size()  &&
            subroot->elements[first_larger_idx] == pair){
      return;
    }
      //case2: if reach the leaf node, insert at given position
    if (subroot->is_leaf){
        auto itr = subroot->elements.begin() + first_larger_idx;
        subroot->elements.insert(itr,pair);
    }else{
        //case3: if not leaf, recusive insert
        insert(subroot->children[first_larger_idx],pair);
        //after insertion at child node, check if full and split_child
        if (subroot->children[first_larger_idx]->elements.size() >= order){
          split_child(subroot,first_larger_idx);
        }
      }
   /*
    if (subroot->is_leaf==false && subroot->elements.size() >= order){
      split_at_root(subroot);
    }
   */
}
...
// template <class K, class V>
// void BTree<K, V>::split_at_root(BTreeNode* subroot)
// {
//    BTreeNode* new_parent = new BTreeNode(false, order);
//    auto itr = subroot->children.begin();
//    new_parent->children.insert(itr,subroot);
//    split_child(new_parent,0);
//
// }
