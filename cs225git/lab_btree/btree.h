...
class BTree
{
  ...
  private:
    ...
    void split_at_root(BTreeNode* parent);
};
...
size_t insertion_idx(const std::vector<T>& elements, const C& val, int start, int end);

template <class T, class C>
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    /* TODO Your code goes here! */
    return insertion_idx(elements,val, 1,int(elements.size()));
}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val, int start, int end)
{
    ...
    if (len >= 1){
      ...
      if (elements[mid-1] == val){
        return mid-1;
      }

      if (val < elements[mid-1]){
        return insertion_idx(elements,val,start,mid-1);
      }

      if (val > elements[mid-1]){

        return insertion_idx(elements,val,mid+1,end);
      }
    }
    return 0;
}
