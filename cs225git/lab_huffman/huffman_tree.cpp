                            ...
                            queue<TreeNode*>& mergeQueue)
{
  ...
  if (singleQueue.empty() && !mergeQueue.empty()){
    TreeNode *smallmerge = mergeQueue.front();
    mergeQueue.pop();
    return smallmerge;
  }

  if (!singleQueue.empty() && mergeQueue.empty()){
    TreeNode *smallsingle = singleQueue.front();
    singleQueue.pop();
    return smallsingle;
  }

  if (!singleQueue.empty() && !mergeQueue.empty()){
    TreeNode *small1 = singleQueue.front();
    TreeNode *small2 = mergeQueue.front();
    if (small2->freq < small1->freq){
      mergeQueue.pop();
      return small2;
    }else{
      singleQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (!singleQueue.empty() || !mergeQueue.empty()){
      TreeNode * tmpleft = removeSmallest(singleQueue,mergeQueue);
      TreeNode * tmpright = removeSmallest(singleQueue,mergeQueue);

      if (tmpleft!=NULL && tmpright!=NULL){
        TreeNode * internal = new TreeNode(
          tmpleft->freq.getFrequency()+tmpright->freq.getFrequency());
        internal->left = tmpleft;
        internal->right = tmpright;
...
//  helper version  (?why is that not correct)
/*
HuffmanTree::TreeNode* HuffmanTree::readTreehelper(BinaryFileReader& bfile){
  TreeNode *internaltmp;
  if (!bfile.hasBits()){
    return NULL;
  }
    ...
    internaltmp = new TreeNode(Frequency(0));
    internaltmp->left = readTreehelper(bfile);
    internaltmp->right = readTreehelper(bfile);
}
return internaltmp;
}
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
  ...
  if (!bfile.hasBits()){
    ...
    head = new TreeNode(Frequency(0));
    head->left = readTreehelper(bfile);
    head->right = readTreehelper(bfile);
  }
  return head;
  }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
  ...
  if (!bfile.hasBits()){
    ...
    head = new TreeNode(Frequency(bfile.getNextByte(),0));
  }
  else{
    head = new TreeNode(Frequency(0));
    //root_ = internal;
    head->left = readTree(bfile);
    head->right = readTree(bfile);
  }
  return head;
  }
